...
void matmul(const float *A, const float *B, float *C, int M, int N, int K) {
  ...
  // size_t gws[2] = {(size_t)M, (size_t)N}, lws[2] = {16, 16};
  size_t gws[2] = {(size_t)(M-1)/4+1, (size_t)N}, lws[2] = {16/4, 16};
  // printf("before gws[0] = %lu, gws[1] = %lu\n", gws[0], gws[1]);
  for (int i = 0; i < 2; ++i) {
  ...
  // printf("after gws[0] = %lu, gws[1] = %lu\n", gws[0], gws[1]);
  // Run kernel
